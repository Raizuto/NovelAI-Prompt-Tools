// ==UserScript==
// @name         NovelAI Prompt Tools for PWA App
// @namespace    https://github.com/Raizuto/NovelAI-Prompt-Tools/tree/main-forked
// @version      5.0.2
// @description  A simple Violentmonkey/Tampermonkey userscript for NovelAI Image Generator that makes prompting easier with a real-time tag suggestion and fast tag weight functionality.
...
(function () {
  ...
  function computeRangeOffsets(rootEl, range) {
    let start = -1, end = -1, currentPos = 0;
    const walker = document.createTreeWalker(rootEl, NodeFilter.SHOW_TEXT, null, false);

    while (walker.nextNode()) {
      const node = walker.currentNode;
      if (node === range.startContainer) start = currentPos + range.startOffset;
      if (node === range.endContainer) end = currentPos + range.endOffset;
      currentPos += node.textContent.length;
    }

    // Fallback for when the cursor is at the very end (past the last text node)
    if (start === -1) start = currentPos;
    if (end === -1) end = currentPos;
  ...
  function adjustInContentEditable(el, increase) {
    ...
    if (!startNode && currentPos + len >= start) {
        startNode = node;
        startOff = Math.max(0, start - currentPos);
    }
    if (!endNode && currentPos + len >= end) {
        endNode = node;
        endOff = Math.max(0, end - currentPos);
    }
    currentPos += len;
}

    if (startNode && endNode) {
    // If endNode is a BR, ensure we don't include it in the replacement range
    if (endNode.tagName === '\n') {
        range.setStart(startNode, startOff);
